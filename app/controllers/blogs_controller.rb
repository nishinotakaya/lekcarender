class BlogsController < ApplicationController
  ...
  def all_blogs_edit
    ...
    one_month = [*@first_day..@last_day]
      @blogs = current_user.blogs.where(start_time: @first_day..@last_day)
        if one_month.count == @blogs.count
          ActiveRecord::Base.transaction do 
            one_month.each { |day| @blogs.create!(start_time: day) }
          end
        end
      @blogs = current_user.blogs.where(start_time: @first_day..@last_day).select("blogs.id", "start_time", "title", "content").group("blogs.id", :start_time)
   end  
